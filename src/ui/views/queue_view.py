...
class QueueRequests(discord.ui.ActionRow[discord.ui.LayoutView]):
    ...
    async def leave_btn(self, interaction: discord.Interaction, button):
        ...
        if await interaction.client.queue.is_in_queue(interaction.user.id):
            ...
            msg = await interaction.followup.send("Removed from queue.", ephemeral=True, wait=True)
            await msg.delete(delay=Messages.DEFAULT_TIMEOUT)
        else:
            msg = await interaction.followup.send("You aren't currently in the queue", ephemeral=True, wait=True)
    ...
    async def position_btn(self, interaction: discord.Interaction, button):
        ...
        if pos is None:
            ...
            try:
                ...
                await msg.delete(Messages.DEFAULT_TIMEOUT)
