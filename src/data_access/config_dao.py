...
from ui.helpers.constants import QUEUE_SCHEDULE, TA_TEXT_CHANNEL_NAME
...
async def _get_queue_times() -> tuple[int, int, int, int]:
    ...
    async with db_manager.get_conn() as conn:
        ...
        async with conn.cursor(DictCursor) as cursor:
            cursor: DictCursor
            await cursor.execute("SELECT open_hour, open_minute, close_hour, close_minute FROM config WHERE name = %s", (QUEUE_SCHEDULE,))
    ...
    async with db_manager.get_conn() as conn:
        ...
        async with conn.cursor() as cursor:
            ...
            await cursor.execute(
                ...
                (open_hour, open_minute, close_hour, close_minute, QUEUE_SCHEDULE)
            )
